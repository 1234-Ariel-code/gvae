...
python3 latent_classification.py \
  ...
  --downsample_d 50000 \
  --assoc_path /work/long_lab/for_Ariel/gwas_results/T2D_gwas.assoc \
  --tped_file /work/long_lab/for_Ariel/files/pruned/T2D_origin.tped \
  --train_vae_epochs 50 \
  --vae_batch_size 256 \
  --batch_size 256 \
  --epochs 120 \
  --cache_latents \
  --out_root /work/long_lab/Ariel_Kemogne/Representation_learning/AJHG/T2D/latent_classification_outputs \
  --make_plots
"""
...
# ------------------------------
# SNP ID readers (for alignment checks / GWAS-top)
# ------------------------------
...
def load_phenotype_auto(phen_path: str):
    """
    Returns:
      y (np.ndarray): labels (0/1 for binary; float for quantitative)
      keep (np.ndarray bool): which rows to keep (exclude missing codes)
      task (str): "classification" or "regression"
      info (dict): quick summary
    """
    df = pd.read_csv(phen_path, delim_whitespace=True, header=None)
    if df.shape[1] < 3:
        raise ValueError(f"Phenotype file must have >=3 columns (FID IID PHENO): {phen_path}")
    ...
    # Common missing codes in genetics
    missing_codes = set([-9, 9, 99, 999])
    keep = np.ones_like(y_raw, dtype=bool)

    # mark NaNs
    if np.issubdtype(y_raw.dtype, np.number):
        keep &= ~np.isnan(y_raw.astype(float, copy=False))
    else:
        # try coerce; anything failing becomes NaN
        y_tmp = pd.to_numeric(pd.Series(y_raw), errors="coerce").to_numpy()
        keep &= ~np.isnan(y_tmp)
        y_raw = y_tmp
    ...
    elif set(uniq.tolist()).issubset({1, 2}):
        task = "classification"
        # WTCCC-style mapping: 1->0 (control), 2->1 (case)
    ...
    info = {
        ...
        "n_kept": int(keep.sum()),
        "n_unique_kept": int(len(np.unique(y_raw[keep]))),
        "min_kept": float(np.min(y_raw[keep])),
        "max_kept": float(np.max(y_raw[keep])),
        "task": task,
    }
    return y, keep, task, info
...
def build_encoder(original_dim: int, latent_dim: int, num_layers: int, *, use_layernorm: bool = False) -> tf.keras.Model:
    ...
    for i in range(num_layers):
        x = layers.Dense(n, activation="relu", name=f"enc_dense_{i}")(x)
        n //= 2

    z_params = layers.Dense(latent_dim * 2, name="z_params")(x)  # mu, log_var
    return tf.keras.Model(inp, z_params, name="encoder")


def build_decoder(original_dim: int, latent_dim: int, num_layers: int) -> tf.keras.Model:
    # mirror of encoder widths
    init = compute_initial_neurons(num_layers, latent_dim)
    widths = []
    n = init
    ...
    inp = layers.Input(shape=(latent_dim,), name="z")
    x = inp
    # reverse widths
    for i, w in enumerate(reversed(widths)):
        x = layers.Dense(w, activation="relu", name=f"dec_dense_{i}")(x)

    # genotype reconstruction (continuous)
    out = layers.Dense(original_dim, activation="linear", name="x_hat")(x)
    return tf.keras.Model(inp, out, name="decoder")


class VAE(tf.keras.Model):
    def __init__(self, encoder: tf.keras.Model, decoder: tf.keras.Model, beta: float = 1.0):
        super().__init__()
...
def train_vae_on_X(
    ...
    *,
    ...
    vae.fit(ds, epochs=epochs, verbose=verbose)
    return vae


# ------------------------------
# Latent feature extraction (from learned VAE)
# ------------------------------
@dataclass
class LatentConfig:
    disease: str
...
def rowwise_quantiles_from_zstack(z_stack: tf.Tensor) -> tf.Tensor:
    """
    z_stack: (K, B, LD)
    Returns: concat(Q25, Q75) => (B, 2*LD)
    """
    sorted_samples = tf.sort(z_stack, axis=0)
    K_ = tf.shape(sorted_samples)[0]
    idx_25 = tf.cast(0.25 * tf.cast(K_, tf.float32), tf.int32)
    idx_75 = tf.cast(0.75 * tf.cast(K_, tf.float32), tf.int32)
    q25 = tf.gather(sorted_samples, idx_25, axis=0)
    q75 = tf.gather(sorted_samples, idx_75, axis=0)
    return tf.concat([q25, q75], axis=-1)


def extract_latent_features_from_vae(X: np.ndarray, cfg: LatentConfig, vae: VAE, batch_size: int) -> np.ndarray:
    feats = []
    for i in range(0, X.shape[0], batch_size):
        xb = tf.convert_to_tensor(X[i:i+batch_size], dtype=tf.float32)
        mu, log_var = vae.encode(xb)

        if cfg.model_type in ("baseline", "betavae"):
            fb = mu  # (B, LD)
        else:
            K = int(max(1, cfg.num_samples))
            z_list = []
            for _ in range(K):
                eps = tf.random.normal(shape=tf.shape(mu), dtype=tf.float32)
                z = mu + eps * tf.exp(0.5 * log_var)
                z_list.append(z)
            z_stack = tf.stack(z_list, axis=0)  # (K, B, LD)
            fb = rowwise_quantiles_from_zstack(z_stack)  # (B, 2*LD)
...
# Classifier
# ------------------------------
def build_classifier(input_dim: int, seed: int = 42) -> tf.keras.Model:
    tf.keras.utils.set_random_seed(seed)
...
def train_classifier(Z: np.ndarray, y: np.ndarray, *, seed: int, epochs: int, batch_size: int, val_size: float):
    Xtr, Xva, ytr, yva = train_test_split(
        Z, y, test_size=val_size, random_state=seed, stratify=y
    )

    clf = build_classifier(input_dim=Z.shape[1], seed=seed)
    callbacks = [
        tf.keras.callbacks.EarlyStopping(monitor="val_auc", mode="max", patience=10, restore_best_weights=True),
        tf.keras.callbacks.ReduceLROnPlateau(monitor="val_auc", mode="max", factor=0.5, patience=5, min_lr=1e-6),
    ]
    ...
    pva = clf.predict(Xva, batch_size=batch_size, verbose=0).reshape(-1)
    auc = roc_auc_score(yva, pva) if len(np.unique(yva)) == 2 else float("nan")
    acc = accuracy_score(yva, (pva >= 0.5).astype(np.int32))
    return hist, {"val_auc_final": float(auc), "val_acc_final": float(acc), "n_val": int(len(yva))}


def build_regressor(input_dim: int, seed: int = 42) -> tf.keras.Model:
    tf.keras.utils.set_random_seed(seed)
    inp = layers.Input(shape=(input_dim,))
    x = layers.Dense(256, activation="relu")(inp)
    x = layers.Dropout(0.25)(x)
...
def train_regressor(Z: np.ndarray, y: np.ndarray, *, seed: int, epochs: int, batch_size: int, val_size: float):
    Xtr, Xva, ytr, yva = train_test_split(
        Z, y, test_size=val_size, random_state=seed
    )

    # (Optional but recommended) standardize target for stable training
    y_mean = float(np.mean(ytr))
    y_std = float(np.std(ytr) + 1e-8)
    ytr_s = (ytr - y_mean) / y_std
    yva_s = (yva - y_mean) / y_std

    reg = build_regressor(input_dim=Z.shape[1], seed=seed)
    callbacks = [
        tf.keras.callbacks.EarlyStopping(monitor="val_rmse", mode="min", patience=10, restore_best_weights=True),
        tf.keras.callbacks.ReduceLROnPlateau(monitor="val_rmse", mode="min", factor=0.5, patience=5, min_lr=1e-6),
    ]
    ...
    pva_s = reg.predict(Xva, batch_size=batch_size, verbose=0).reshape(-1)
    pva = pva_s * y_std + y_mean  # back to original scale
    ...
    # Pearson corr (optional but useful)
    if np.std(pva) > 0 and np.std(yva) > 0:
        corr = float(np.corrcoef(yva, pva)[0, 1])
    else:
        corr = float("nan")

    # Save these into history too (so your plotting can use them)
    hist.history.setdefault("val_r2", [])
    hist.history.setdefault("val_corr", [])
    hist.history["val_r2"].append(r2)
    hist.history["val_corr"].append(corr)
...
def plot_per_config_models(history_dir: str, out_dir: str, disease: str, choose_beta: str = "auto"):
    ...
    Classification:
      ...
      - betaVAE selection by best peak val_auc (choose_beta="best_auc")

    Regression:
      - produces per-config plots for val_rmse, val_mae, val_r2
      - betaVAE selection by best (lowest) min val_rmse (choose_beta="best_rmse")

    choose_beta:
      - "auto": classification -> best_auc, regression -> best_rmse
      - "best_auc"
      - "best_rmse"
      - "beta_1.0" (use beta=1.0 if present)
    """
    import os
    import numpy as np
    import pickle
    ...
    for fn in os.listdir(history_dir):
        ...
        for k in ["val_accuracy", "val_auc", "val_rmse", "val_mae", "val_r2"]:
            hist.setdefault(k, [])
    ...
    # 2) Detect task type
    # -------------------------
    # If any run has regression metric values, treat as regression
    has_reg = any((len(r["hist"].get("val_rmse", [])) > 0) for r in runs)
    task = "regression" if has_reg else "classification"

    if choose_beta == "auto":
        choose_beta = "best_rmse" if task == "regression" else "best_auc"

    # -------------------------
    # Helpers: peak/min metric
    # -------------------------
    def peak(vals):
    ...
    # -------------------------
    # 3) Index runs for lookup
    # -------------------------
    baseline_map = {}   # (LD, L) -> run
    qg_map = {}         # (LD, NS, L) -> run
    beta_map = {}       # (LD, L) -> list of runs
    ...
    def pick_beta_run(LD, L):
        ...
        if choose_beta == "best_rmse":
            best = None
            best_score = float("inf")
            for r in lst:
                score = minval(r["hist"].get("val_rmse", []))  # lower better
                if score < best_score:
                    best_score = score
                    best = r
            return best
    ...
    def plot_metric(metric_key: str, cfg_key, baseline_run, qg_run, beta_run):
        ...
        def safe_plot(run, label):
            vals = run["hist"].get(metric_key, []) if run is not None else []
    ...
    if task == "classification":
        ...
        for cfg in all_configs:
            ...
            fig1 = plot_metric("val_accuracy", cfg, baseline_run, qg_run, beta_run)
            fig1.savefig(os.path.join(acc_dir, f"{disease}_LD{LD}_NS{NS}_L{L}_val_accuracy.png"),
                         dpi=300, bbox_inches="tight")
            plt.close(fig1)

            fig2 = plot_metric("val_auc", cfg, baseline_run, qg_run, beta_run)
            fig2.savefig(os.path.join(auc_dir, f"{disease}_LD{LD}_NS{NS}_L{L}_val_auc.png"),
                         dpi=300, bbox_inches="tight")
            plt.close(fig2)

        print(f"[OK] Saved per-config CLASSIFICATION figs in:\n  {acc_dir}\n  {auc_dir}")

    else:
        rmse_dir = os.path.join(out_dir, "per_config_rmse")
        mae_dir  = os.path.join(out_dir, "per_config_mae")
        r2_dir   = os.path.join(out_dir, "per_config_r2")
        ...
        for cfg in all_configs:
            ...
            fig1 = plot_metric("val_rmse", cfg, baseline_run, qg_run, beta_run)
            fig1.savefig(os.path.join(rmse_dir, f"{disease}_LD{LD}_NS{NS}_L{L}_val_rmse.png"),
                         dpi=300, bbox_inches="tight")
            plt.close(fig1)

            fig2 = plot_metric("val_mae", cfg, baseline_run, qg_run, beta_run)
            fig2.savefig(os.path.join(mae_dir, f"{disease}_LD{LD}_NS{NS}_L{L}_val_mae.png"),
                         dpi=300, bbox_inches="tight")
            plt.close(fig2)

            fig3 = plot_metric("val_r2", cfg, baseline_run, qg_run, beta_run)
            fig3.savefig(os.path.join(r2_dir, f"{disease}_LD{LD}_NS{NS}_L{L}_val_r2.png"),
                         dpi=300, bbox_inches="tight")
            plt.close(fig3)
...
def main():
    ...
    ap.add_argument("--out_root", default="latent_classification_outputs", type=str)
    ...
    geno_csv = os.path.join(base_path, f"{disease}_filtered.csv")
    phen_path = os.path.join(base_path, f"{disease}_origin.phen")

    if args.tped_file is None:
        args.tped_file = os.path.join(base_path, f"{disease}_origin.tped")
    if args.assoc_path is None:
        args.assoc_path = f"/work/long_lab/for_Ariel/gwas_results/{disease}_gwas.assoc"
    ...
    if keep.shape[0] != X.shape[0]:
        raise ValueError(
            f"Phenotype length ({keep.shape[0]}) != Genotype N ({X.shape[0]}).\n"
            f"Make sure CSV individuals align with .phen order."
        )

    X = X[keep]
    y = y_all[keep]

    if args.model_type == "betavae" and args.beta is None:
        raise ValueError("--beta is required when --model_type=betavae")
    ...
    cfg = LatentConfig(
        ...
        beta=args.beta if args.model_type == "betavae" else None,
    )

    out_root = args.out_root
    hist_dir = os.path.join(out_root, "class_history")
    met_dir = os.path.join(out_root, "class_metrics")
    lat_dir = os.path.join(out_root, "class_latents")
    sum_dir = os.path.join(out_root, "class_summary")
    fig_dir = os.path.join(out_root, "figures")
    for d in [hist_dir, met_dir, lat_dir, sum_dir, fig_dir]:
        os.makedirs(d, exist_ok=True)

    beta_tag = "NA" if cfg.beta is None else str(cfg.beta)
    run_tag = f"{disease}_{cfg.model_type}_LD{cfg.latent_dim}_NS{cfg.num_samples}_L{cfg.num_layers}_B{beta_tag}"
    cache_path = os.path.join(lat_dir, f"{run_tag}.npy")

    # --------------------------
    # 1) Train VAE weights here
    # --------------------------
    ...
    if args.cache_latents and os.path.exists(cache_path):
        ...
        print(f"[OK] Loaded cached latents: {cache_path}  shape={Z.shape}")
    else:
        print("[STEP] Extracting latent features...")
        Z = extract_latent_features_from_vae(X=X, cfg=cfg, vae=vae, batch_size=args.batch_size)
        if args.cache_latents:
            np.save(cache_path, Z)
            print(f"[OK] Saved cached latents: {cache_path}  shape={Z.shape}")
    ...
    if task == "classification":
        ...
        history_dict.setdefault("val_auc", [])
        history_dict.setdefault("val_accuracy", [])
    else:
        hist, final_metrics = train_regressor(
            Z=Z, y=y.astype(np.float32), seed=args.seed, epochs=args.epochs, batch_size=args.batch_size, val_size=args.val_size
        )
        history_dict = hist.history
        # for plotting
        history_dict.setdefault("val_rmse", [])
        history_dict.setdefault("val_mae", [])
        history_dict.setdefault("val_loss", [])
    ...
    print(f"[OK] Saved metrics : {met_path}")
