...
r2_order <- c(
  "COL", "LUN", "HDL", "PRC", "BRC", "EOS", "BMI", "HGT", "LDL", "ASD",
  "T1D", "T2D", "CD", "RA", "HT", "BD", "CAD", "ALZ"
)

mse_order <- c(
  "ALZ", "CD", "BD", "T2D", "CAD", "RA", "T1D", "HT",
  "EOS", "ASD", "HDL", "PRC", "COL", "LUN", "BMI", "HGT", "BRC", "LDL"
)

# Classification panels
auc_order <- c(
  "CAD", "RA", "COL", "CD", "T2D", "BD", "HT", "PRC", "LUN", "EOS",
  "ALZ", "ASD", "BMI", "BRC", "HDL", "HGT", "LDL", "T1D"
)

acc_order <- c(
  "T2D", "RA", "CAD", "CD", "BD", "COL", "LUN", "HT", "PRC", "EOS",
  "ALZ", "ASD", "BMI", "BRC", "HDL", "HGT", "LDL", "T1D"
)
...
p_r2 <- make_panel(
  r2_long,
  title = "a  R2 across diseases",
  xlab = expression(R^2 ~ "(higher is better)"),
  xlim_max = 0.66,
  xticks = seq(0, 0.6, by = 0.1),
  ylab = "Disease"
)

p_mse <- make_panel(
  mse_long,
  title = "b  MSE across diseases",
  xlab = "MSE (lower is better)",
  xlim_max = 0.75,
  xticks = seq(0, 0.7, by = 0.1),
  ylab = NULL
) +
  theme(
...
p_auc <- make_panel(
  auc_long,
  title = "c  AUC across diseases",
  xlab = "AUC (higher is better)",
  xlim_max = 0.90,
  xticks = seq(0, 0.9, by = 0.1),
  ylab = "Disease"
)

p_acc <- make_panel(
  acc_long,
  title = "d  Accuracy across diseases",
  xlab = "Accuracy (higher is better)",
  xlim_max = 1.05,
  xticks = seq(0, 1.0, by = 0.1),
  ylab = NULL
) +
  theme(
    axis.text.y = element_blank(),
...
# ------------------------------------------------------------
# 8. COMBINE 2x2 PANELS
# ------------------------------------------------------------
final_plot <- (p_r2 | p_mse) / (p_auc | p_acc) +
...
# ------------------------------------------------------------
# 10. SAVE HIGH-RESOLUTION OUTPUTS
# ------------------------------------------------------------
save_pdf_device <- if (capabilities("cairo")) cairo_pdf else "pdf"

ggsave(
  filename = "Figure_R2_MSE_AUC_Accuracy_across_diseases_high_impact.pdf",
  plot = final_plot,
  width = 16,
  height = 11,
  units = "in",
  dpi = 600,
  device = save_pdf_device,
  bg = "white"
)

ggsave(
  filename = "Figure_R2_MSE_AUC_Accuracy_across_diseases_high_impact.png",
  plot = final_plot,
  width = 16,
  height = 11,
  units = "in",
...
ggsave(
  filename = "Figure_R2_MSE_AUC_Accuracy_across_diseases_high_impact.tiff",
  plot = final_plot,
  width = 16,
  height = 11,
  units = "in",
